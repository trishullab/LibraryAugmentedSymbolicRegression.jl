...
@unstable function cache_stats(cache::SuggestionCache)
    return lock(cache.lock) do
...
A ceiling on how many LLM calls one iteration may make.
...
LLM operators fall back to their symbolic counterparts for the rest of the iteration,
...
Reserve one call. Returns `false` once the iteration's allowance is spent.
"""
...
"""Start a fresh allowance; called at each iteration boundary."""
function reset_budget!(budget::CallBudget)
    budget.used[] = 0
    budget.denied[] = 0
    return nothing
end
